...
class TreeView():
    ...
    def run_command_for_file_under_cursor(self, command, args=None):
...
class TreeViewAddFileCommand(TreeView, sublime_plugin.WindowCommand):
    ...
    def run(self):
        self.run_command_for_file_under_cursor('new_file_at', {'dirs': []})
...
class TreeViewToggleCommand(TreeView, sublime_plugin.WindowCommand):
    ...
    def run(self, sidebar_currently_focused=None):
        # Args:
        #   :sidebar_currently_focused (bool): Flag to indicate if the sidebar
        #       is currently focused. Required for < 3115, becuase it doesn't
        #       have the is_sidebar_visible() api.
